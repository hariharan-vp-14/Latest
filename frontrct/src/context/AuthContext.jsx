...
export const AuthProvider = ({ children }) => {
    ...
    const restoreSession = async () => {
      try {
        ...
        if (!storedRole) {
          ...
          console.log('ℹ️ No stored role - user not previously logged in');
          setLoading(false);
          return;
        }
        ...
        if (storedAccessToken) {
          console.log('✅ Using stored access token for quick session restore');
          ...
          try {
            ...
            console.warn('⚠️ Token refresh failed, but using stored token:', refreshErr.message);
          }
        ...
        console.log('🔄 No stored access token - attempting refresh via cookie');
        const response = await apiService.refreshToken(storedRole);
        if (response?.accessToken && response?.user) {
          apiService.setAuth(response.accessToken);
          setAccessToken(response.accessToken);
          ...
          localStorage.setItem('userRole', role);
          localStorage.setItem('accessToken', response.accessToken);
          console.log('✅ Session restored successfully for role:', role);
        }
      } catch (err) {
  ...
  const handleLogin = async (apiCall, role) => {
    ...
    try {
      ...
      console.log('💾 Stored role and token in localStorage');
      console.log('📋 API Service token set:', apiService.accessToken ? `✓ (${apiService.accessToken.length} chars)` : '✗ Missing');
